...
for peak in peaks['peak_id']:
    ...
    try:
        html = get_members_data('https://www.himalayandatabase.com/scripts/peakdead.php', peak)
        death_count = html.find_all('table')[3].text
        death_count = int(re.findall(r'Death Count = \d*', death_count)[0].split('=')[1].strip())
        print(f'Found {death_count} deaths for {peak}')
    except:
        print(f'Not Found Any Deaths on {peak}')
        continue
    ...
    for row in table_rows:
        death = {
            ...
            'is_summiter': row[9].text,
            'cause_of_death': row[10].text
        }
        deaths.append(death)

    df = pd.DataFrame(deaths)
    df.to_csv(f'C:/Users/vlad/Desktop/himalayan_db/deaths_data/{peak}_deaths.csv', index=False)
